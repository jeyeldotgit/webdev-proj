...
@section('title', 'Edit Assignment - SproutLMS')

@section('content')
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-6">
        <a href="{{ route('assignments.show', [$course, $assignment]) }}" class="text-primary-600 hover:text-primary-700 mb-4 inline-block">
            ← Back to Assignment
        </a>
        <h1 class="text-3xl font-bold text-neutral-900">Edit Assignment</h1>
        <p class="text-neutral-600 mt-1">Course: {{ $course->title }}</p>
    </div>

    <div class="bg-white rounded-xl shadow-sm border border-neutral-200 p-8">
        <form method="POST" action="{{ route('assignments.update', [$course, $assignment]) }}">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="6"
                    class="w-full px-4 py-2 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-primary-600 focus:border-primary-600">{{ old('description', $assignment->description) }}</textarea>
                @error('description')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="number" name="max_score" id="max_score" value="{{ old('max_score', $assignment->max_score) }}" required min="1" max="1000"
                    class="w-full px-4 py-2 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-primary-600 focus:border-primary-600">
                @error('max_score')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="flex gap-4">
                ...
                <a href="{{ route('assignments.show', [$course, $assignment]) }}" class="border border-neutral-300 text-neutral-700 px-6 py-3 rounded-lg hover:bg-neutral-50 transition-colors font-semibold">
                    Cancel
                </a>
            </div>
        </form>
